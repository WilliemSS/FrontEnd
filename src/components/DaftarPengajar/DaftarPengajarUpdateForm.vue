<template>
  ...
  <button type="submit" class="btn btn-primary">Update Pengajar</button>&nbsp;
...
<script>
  ...
  methods: {
    ...
    async fetchPengajarDetails(){
        const Id_pengajar=this.$route.params.Id_pengajar;
        try{
            const response = await axios.get(`http://localhost:8000/api/daftar-pengajar/${Id_pengajar}`);
            //Assign the fetched mata kuliah data to component data fields
            this.selectedNIM=response.data.NIM;
            this.selectedNamaMahasiswa=response.data.Nama_mahasiswa;
            this.selectedNamaMatkul=response.data.Nama_matkul;
            this.selectedSKS=response.data.SKS;
            this.selectedNID=response.data.NID;
            this.selectedNamaDosen=response.data.Nama_dosen
        }catch(error){
            console.error('Error fetching data Dosen details',error);
        }
    },
    async updatePengajar() {
      // Create an object with form data
      const updatedDataPengajar = {
      ...
      try {
        const Id_pengajar=this.$route.params.Id_pengajar;
        const response = await axios.put(`http://localhost:8000/api/daftar-pengajar/${Id_pengajar}`, updatedDataPengajar);

        if (response.status === 200) { // Check if updated was successful 
          console.log('Data Pengajar updated successfully');
          this.showSuccessMessage=true;
          // Reset the form fields after successful submission
        } else {
            console.error('Failed to update Data Pengajar with status:', response.status);
